...
namespace flex_sync {
  ...
  class LiveSync <SyncT, MsgPack<MsgTypes...> > {
    ...
    public:
      ...
      LiveTopic(const std::string &topic, ros::NodeHandle &nh,
  ...
  public:
    ...
    typedef std::tuple<
      std::vector<
        std::shared_ptr<
          LiveTopic<const MsgTypes>>>...> TupleOfTopicVec;
    typedef typename SyncT::Callback Callback;

    LiveSync(const ros::NodeHandle &nh,
  ...
  private:
      ...
      int operate(LiveSync<SyncT> *liveSync) const
        {
          ...
          for (const std::string &topic: topics[I]) {
            ...
            typedef typename get_type<
              I, TupleOfTopicVec>::type::value_type::element_type LiveTopicT;
            std::shared_ptr<LiveTopicT> lt(
              new LiveTopicT(topic, nh, qs, sync));
    ...
    template <int N, typename... Ts> struct get_type;
